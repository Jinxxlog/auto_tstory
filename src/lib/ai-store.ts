...
export function aiStore(store: ReturnType<typeof openStore>) {
  ...
  return { connection, setConnection, jobs, job, put,
    ...
    enqueue(id: string, model: string, selection: string, instruction: string) { return tx(() => {
      ...
      const value: AiJob = { id: randomUUID(), state: 'queued', step: '생성 대기', draft, model, selection, instruction, attempts: 0, cancel: false, createdAt: new Date().toISOString() }; put(value); return value;
    }); },
