...
export type AiJob = { id: string; state: 'queued' | 'running' | 'succeeded' | 'failed' | 'cancelled'; step: string; draft: Draft; model: string; selection: string; instruction: string; style?: StyleSnapshot; references?: Reference[]; attempts: number; cancel: boolean; output?: AiOutput; usage?: { input: number; output: number; total: number }; appliedVersion?: number; createdAt: string };
