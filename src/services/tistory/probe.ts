...
export class TistoryProbe {
  ...
  constructor(blog?: string) { this.blog = blog ? normalizeBlogUrl(blog) : undefined; }

  async start() {
    await mkdir(localRoot, { recursive: true });
    this.context = await chromium.launchPersistentContext(path.join(localRoot, 'tistory-profile'), {
  ...
  private watchDialogs(page: Page) {
    page.on('dialog', () => console.log('브라우저 확인창이 열렸습니다. 내용을 보고 브라우저에서 직접 처리하세요.'));
  }
  ...
  async inspect() {
      ...
      const dom = await frame.evaluate(() => {
        ...
        return {
          ...
          controls: Array.from(document.querySelectorAll('button, a, input, textarea, select, [role="button"], [role="option"], [contenteditable="true"], iframe'))
            .filter(el => visible(el) || el.getAttribute('type') === 'file')
            .map(el => ({
    ...
    await writeFile(path.join(localRoot, 'last-inspection.json'), JSON.stringify(result, null, 2), 'utf8');
    return result;
  }

  async editor() {
    const page = this.requireBlog();
    const link = page.getByRole('link', { name: '글쓰기', exact: true });
  ...
  async screenshot() {
    await this.requireBlog().screenshot({ path: path.join(localRoot, 'last-page.png'), fullPage: true });
