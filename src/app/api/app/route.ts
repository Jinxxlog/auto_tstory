import { openStore, validateDraft } from '../../../lib/store';
...
import { checkEnvironment } from '../../../../scripts/environment.mjs';
...
export async function GET(request: Request) {
  ...
  try { const ai = aiStore(store); const styles=styleStore(store); return Response.json({ photos: photoAnalysisStore(store).jobs().slice(0, 30), connections: connectionSummary(store.settings(), ai.connection()), references: referenceStore(store).list(), checks: checkStore(store).list(), styles:{defaults:styles.defaults(),sources:styles.sources(),profiles:styles.profiles(),jobs:styles.jobs().slice(0,30)}, ai: { connection: ai.connection(), jobs: ai.jobs().slice(0,50) }, settings: store.settings(), drafts: store.drafts(), assets: store.assets(), jobs: store.jobs().map(job => ({ ...job, result: job.result || job.snapshot.postUrl || null })), workerOnline: store.workerOnline() }); } finally { store.db.close(); }
}
export async function POST(request: Request) {
  ...
  while (true) { const chunk = await reader.read(); if (chunk.done) break; size += chunk.value.length; if (size > 2_000_000) { await reader.cancel(); return Response.json({ error: '원고가 너무 큽니다.' }, { status: 413 }); } chunks.push(chunk.value); }
  body = Buffer.concat(chunks).toString('utf8');
  const store = openStore();
  try {
    const input = JSON.parse(body);
    switch (input.action) {
      case 'photo-analyze': if (typeof input.id !== 'string' || typeof input.model !== 'string') throw new Error('사진 분석 입력을 확인하세요.'); return Response.json(await photoAnalysisStore(store).enqueue(input.id, input.model, input.ids));
      case 'photo-cancel': photoAnalysisStore(store).cancel(String(input.id)); return Response.json({ok:true});
      case 'photo-apply': return Response.json(photoAnalysisStore(store).apply(String(input.id)));
      ...
      case 'style-default': if (typeof input.kind !== 'string' || typeof input.profileId !== 'string') throw new Error('기본 문체를 확인하세요.'); return Response.json(styleStore(store).setDefault(input.kind, input.profileId));
      ...
      case 'ai-generate': {
        ...
        return Response.json(aiStore(store).enqueue(input.id, input.model, input.selection, input.instruction, typeof input.profileId==='string'?input.profileId:'@blog', typeof input.targetBlockId === 'string' ? input.targetBlockId : ''));
      }
      ...
      case 'preview': if (input.draft) return Response.json({ html: renderDraft(validateDraft(input.draft)) }); if (typeof input.markdown !== 'string' || input.markdown.length > 100000) throw new Error('본문 길이를 확인하세요.'); return Response.json({ html: renderMarkdown(input.markdown) });
      case 'settings': {
      ...
      case 'diagnostics': return Response.json(await checkEnvironment({ checkPort: false, production: process.env.NODE_ENV === 'production' }));
      ...
      case 'verify': if (input.url !== undefined && typeof input.url !== 'string') throw new Error('글 주소를 입력하세요.'); return Response.json(store.verify(String(input.id), input.url));
      case 'resolve-unpublished': return Response.json(store.resolveUnpublished(String(input.id), input.confirmed === true));
